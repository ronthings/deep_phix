...
def withincheck(position, gene_start, gene_end, length): ###### we should be able to delete this if I can figure out how to get position in feature....
...
def main():
    ...
    parser.add_argument("gbk_file", type=str, help="Genbank file")
...
# CodonTable.unambiguous_dna_by_id[11]

    # read Genbank file to extract sequence
    from Bio import SeqIO
    record = SeqIO.read(args.gbk_file, "genbank")
    genome = str(record.seq)
    length = len(record)
    print("Measured genome length of", record.name, "is", length, "bases.")
    # need to add way to break out if >1 sequence record!!!
    genes = {} # genecoords, subsequences > genes
    if record.features:
        for feature in record.features:
            if feature.type == "CDS":
                gen = feat.qualifiers['gene'][0] # add error check to quit if >1 gene!!
                genes[gen] = feature

    # feat.extract(record) = sequence
    # feat.qualifiers['translation'] = translated - could do on the fly too
    ...
    codons = { # we may be able to delete this....
        ...
        for line in file:
            if not line:
                ...
                for base, num in zip(altnucs, ad_num):
                    ...
                    for gen, feature in genes.items(): #gen, coord in genecoords.items()
                        #beg, ter = coord # tuple unpacking
                        if (refpos-1) in feature:
